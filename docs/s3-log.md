...
| 2026-09-18 | `f566744` | Two more public clauses replayed — Fortis/Auris (ten milestones: the trigger is an event, not a figure) and In re SwervePay (a pro-rated "up to" amount across two revenue definitions). Five public clauses now: one fits, four refused. `docs/validation.md` adds the milestone finding and the case this product cannot help with (a baseline misrepresented before signing). |
